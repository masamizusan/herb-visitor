          ...
          <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 snap-x">
            {seasonalPlants.map((plant) => (
              <Link
                key={plant.id}
                href={`/plants/${plant.id}`}
                className="flex-shrink-0 w-36 bg-white rounded-2xl shadow-sm card-hover snap-start"
              >
                <div className="aspect-[4/3] bg-green-100 relative rounded-t-2xl overflow-hidden">
                  <PlantImage
                    ...
                    iconSize={28}
                  />
                  <span className="absolute top-2 left-2 bg-herb-primary/90 backdrop-blur-sm rounded-sm px-2 py-0.5 text-[10px] font-semibold text-white">
                    見頃
                  </span>
                </div>
                <div className="p-2.5">
